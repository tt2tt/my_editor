...
from PySide6.QtWidgets import (QApplication, QCheckBox, QFileDialog, QMainWindow,
                               QMenu, QTabWidget, QToolBar, QTreeView, QVBoxLayout,
                               QWidget, QSplitter, QLineEdit, QPushButton, QFileSystemModel)
        ...
        if file_editor is not None:
            if file_editor.current_file is None:
                file_path, _ = QFileDialog.getSaveFileName(
                    self, "ファイルを保存", "", "All Files (*);;Text Files (*.txt)"
                )
                if file_path:
                    ...
                    if isinstance(current_widget, FileEditor):
                        ...
                        internal_tabs.setTabText(
                            internal_tabs.indexOf(file_editor), file_name
                        )
            else:
        ...
        if file_editor is not None:
            ...
            if pattern:
                ...
                if self.regex_checkbox.isChecked():
                    ...
                    start_pos = (
                        self.last_match_end
                        if pattern == self.last_search_pattern
                        else 0
                    )
